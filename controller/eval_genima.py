...
class GenimaEvalWorkspace(Workspace):
    def __init__(
        ...
        # Initialize the diffusion agent
        self.diffusion_agent = hydra.utils.instantiate(
            self.eval_cfg.diffusion_agent,
            self.eval_cfg,
        )
    ...
    def eval_checkpoints(self, eval_ckpts):
        ...
        for run_id, eval_ckpt in enumerate(eval_ckpts):
            ...
            while eval_until_episode(run_episode):
                ...
                diffusion_seed = self.eval_cfg.diffusion_seed
                self.generator = [
                    torch.Generator(device=self.eval_cfg.device).manual_seed(
                        diffusion_seed
                    )
                ]
                ...
                while not termination:
                    num_frames = self.train_cfg.frame_stack
                    cameras = self.train_cfg.env.cameras

                    # Gather rgbs (frames x cameras) and prompts
                    rgbs = []
                    for camera in cameras:
                        for t in range(num_frames):
                            rgb = Image.fromarray(
                                np.transpose(obs[f"{camera}_rgb"][t], (1, 2, 0))
                            )
                            rgbs.append(rgb)
                    ...
                    for t in range(num_frames):
                        prompt = f"tiled perspectives of a robot arm executing '{goal}'"
                    ...
                    # Tile images
                    tiled_images = tile_images(rgbs, num_frames)
                    ...
                    if self.eval_cfg.save_input_image:
                        for tidx, timg in enumerate(tiled_images):
                            timg.save(
                                os.path.join(
                                    self.eval_cfg.save_image_path,
                                    f"input_ep{global_episode}_step{episode_step}_frame{tidx}.png",
                                )
                            )
                    ...
                    with torch.inference_mode(), tiger_utils.eval_mode(
                        ...
                        target_images = self.diffusion_agent.infer(
                            images=tiled_images,
                            prompts=prompts,
                            negative_prompts=negative_prompts,
                            num_inference_steps=self.eval_cfg.num_diffusion_steps,
                            guidance_scale=self.eval_cfg.guidance_scale,
                            generator=self.generator * len(tiled_images),
                        )
                        ...
                        if self.eval_cfg.save_gen_image:
                            for tidx, timg in enumerate(target_images[0]):
                                timg.save(
                                    os.path.join(
                                        self.eval_cfg.save_image_path,
                                        f"gen_target_ep{global_episode}_step{episode_step}_frame{tidx}.png",
                                    )
                                )

                        # Untile target images
                        untiled_images = untile_images(
                            target_images[0],
                            cameras,
                            self.diffusion_agent.transform_to_half_resolution,
                        )

                        # Update observation with target images for each camera
                        obs["wrist_rgb"] = untiled_images["wrist"]
                        obs["front_rgb"] = untiled_images["front"]
                        obs["right_shoulder_rgb"] = untiled_images["right_shoulder"]
                        obs["left_shoulder_rgb"] = untiled_images["left_shoulder"]
                        ...
                        # Save timings
                        timings["gen_time"].append(gen_time)
                ...
                metrics = {
                    ...
                    "gen_time": np.mean(timings["gen_time"]),
                ...
                self.logger.log_metrics(metrics, global_episode, prefix="eval_genima")
                ...
                print(
                    (
                        ...
                        f"| Gen Time: {np.mean(timings['gen_time']):.4f}s\t"

#!/usr/bin/env python
# Credit: Based on https://github.com/huggingface/diffusers/blob/main/examples/controlnet/train_controlnet.py
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to pretrained controlnet model or model "
    ...
    parser.add_argument(
        ...
        type=str,
        default="train_controlnet_genima",
        help=(
...
def log_validation(
    ...
    variation = f"variation{args.variation}"
    ...
    for render_img in imgs:
        ...
        validation_image = image_transform(validation_image)
        gt_image = image_transform(gt_image)
        ...
        if args.tiled:
            validation_prompt = f"tiled perspectives of a robot arm executing \
            {np.random.choice(descriptions)}"
        else:
            validation_prompt = f"a robot arm executing '\
                {np.random.choice(descriptions)}' \
        ...
        for _ in range(num_samples):
            with torch.autocast("cuda"):
                image = pipeline(
                    ...
                    num_inference_steps=4,  # fixed to 4 steps
                    generator=generator,
                    guidance_scale=0.0,
                ).images[0]
...
def augment_data(args, batch):
    ...
    if args.augmentations:
        ...
        if "affine" in augs_list:
            ...
            conditioning_images = transforms.functional.affine(
                conditioning_images, *random_affine_transform
            )
        ...
        if "crop" in augs_list:
            ...
            conditioning_image = F.pad(conditioning_images, padding, mode=padding_mode)
            conditioning_images = apply_random_crop(conditioning_image, crop_params)
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    # or specify a Dataset from the hub
    ...
    # Preprocessing ground-truth images
    ...
    # Preprocessing conditioning images
...
def main(args):
    ...
    if accelerator.unwrap_model(controlnet).dtype != torch.float32:
        raise ValueError(
            f"Controlnet loaded as datatype \
            {accelerator.unwrap_model(controlnet).dtype}. \
            {low_precision_error_string}"
        )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {"name": args.report_name}},
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                if args.timestep_spacing == "turbo_timesteps":
                    ...
                    steps = [999, 749, 499, 249, 0]
                    timesteps = torch.tensor(
                        [random.choice(steps) for _ in range(bsz)],
